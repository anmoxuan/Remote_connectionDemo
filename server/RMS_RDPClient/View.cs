...
namespace Vnc.Viewer
{
    ...
    internal abstract class View : Form
    {
        ...
        {
            MenuItem item = (MenuItem)sender;
            if (item.Text == App.GetStr("系统自适应") && connOpts.ViewOpts.PixelSize != PixelSize.Unspec)
            {
                connOpts.ViewOpts.PixelSize = PixelSize.Unspec;
                conn.IsFmtChgPending = true;
            }
            else if (item.Text == App.GetStr("8位") && connOpts.ViewOpts.PixelSize != PixelSize.Force8Bit)
            {
                connOpts.ViewOpts.PixelSize = PixelSize.Force8Bit;
                conn.IsFmtChgPending = true;
            }
            else if (item.Text == App.GetStr("16位") && connOpts.ViewOpts.PixelSize != PixelSize.Force16Bit)
            {
        ...
        protected void CheckPixelSize(Menu menu)
        {
            ...
            {
                MenuItem item = menu.MenuItems[i];
                if (item.Text == App.GetStr("系统自适应"))
                    serverDecides = item;
                else if (item.Text == App.GetStr("8位"))
                    force8Bit = item;
                else if (item.Text == App.GetStr("16位"))
                    force16Bit = item;
            }
        ...
        internal View(Conn conn, ConnOpts connOpts, UInt16 width, UInt16 height)
            : base()
        {
            this.conn = conn;
            ...
            servScalingMenu.MenuItems.Add(item);
            CheckServScaling(servScalingMenu);
            pixelSizeMenu.Text = App.GetStr("像素尺寸");
            viewMenu.MenuItems.Add(pixelSizeMenu);
            item = new MenuItem();
            item.Text = App.GetStr("系统自适应");
            item.Click += pixelSizeHdr;
            pixelSizeMenu.MenuItems.Add(item);
            item = new MenuItem();
            item.Text = App.GetStr("8位");
            item.Click += pixelSizeHdr;
            pixelSizeMenu.MenuItems.Add(item);
            item = new MenuItem();
            item.Text = App.GetStr("16位");
            item.Click += pixelSizeHdr;
            pixelSizeMenu.MenuItems.Add(item);
            CheckPixelSize();
            item = new MenuItem();
        ...
        private void InitializeComponent()
        {
            ...
            this.Icon = ((System.Drawing.Icon)(global::VNCClientTest.Properties.Resources.托盘在线));
            this.Name = "View";
