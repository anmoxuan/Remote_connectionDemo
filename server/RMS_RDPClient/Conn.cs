...
namespace Vnc.Viewer
{
    ...
    internal class Conn
    {
        ...
        private void NegoProtoVer(string info)
        {
            ...
            if (!RfbProtoUtil.IsValidVerMsg(verMsg))
            {
                if (string.IsNullOrEmpty(info))
                    ...
                    throw new WarnEx(App.GetStr(info + "¡¨Ω” ß∞‹£°"));
            }
        ...
        internal void Run(string ip, string port, bool readOnly, string info)
        {
            try
            {
                // Get connection details from the user.
                if (opts == null)
                    GetConnDetails(ip, port, readOnly);
